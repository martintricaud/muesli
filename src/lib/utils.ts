...
export const lensProps = props => R.lens(R.props(props), liftAssoc(props));
...
// let lensView = field => obj => lerp([obj['c0'],obj['c1']],[0,1000])(obj[field])
function lensView(field:string){
  return function(obj:Record<string,number>){
    return lerpBis([obj['c0'],obj['c1']],[0,1000],obj[field])
  }
}
function lensSet(field:string){
  return function(val:number, obj:Record<string,number>){
    return R.assoc(field,lerpBis([0,1000],[obj['c0'],obj['c1']],val),obj)
  }
}
...
export const updateRecordField2 = (field, partialUpdateRecord) => record => R.mergeWith((v, f) => R.modifyPath(field, f, v), record, partialUpdateRecord)
  
export const updateRecordField = (field, partialUpdateRecord) => record => R.mergeWith((v, f) => R.modifyPath(field, f, v), record, partialUpdateRecord)
export const setRecordField = (field, partialRecord) => updateRecordField(field, R.map(x => {return {[field]: R.always(x)}}, partialRecord))
//equivalent of ModifyPath lifted to Records
export const updateRecord = (partialUpdateRecord) => record => R.mergeDeepWith(R.call, partialUpdateRecord, record)
//equivalent of AssocPath lifted to Records
export const setRecord = (partialRecord) => updateRecord(R.map(x=>R.always(x),partialRecord))
