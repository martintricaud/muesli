<script>
  ...
  */
  ...
  function fix_effect(ev) {
    //console.log(ev)
    let e = ev?.detail;
    const path = U.stringToPath(ev?.detail?.target?.dataset?.path??"");
    const store = ev?.detail?.target?.dataset?.store;
  ...
  <div class="ui">
    ...
    <ul class="sliders" style="overflow:{noscroll ? 'hidden' : 'scroll'}">
      ...
      {#each Object.entries($P) as [key, { a, b, c0, c1, z, locked }]}
        <li class:foo={locked} class="erasable {ff}" on:click={click_effect} data-path={key} data-store="2">
